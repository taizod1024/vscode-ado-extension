...
- **Epic/Issue/Task 作成** (`ado-ext.openCreateEpic/openCreateIssue/openCreateTask`)
  - 新しいワークアイテムを作成する
...
- **Work Item を Copilot に送信** (`ado-ext.sendWorkItemToCopilot`)
  ...
  - `workitem_active` コンテキストのインラインアクションとして表示（`$(copilot)` アイコン）
...
- **Work Item に対応するブランチを作成** (`ado-ext.createBranchForWorkItem`)
  ...
  - `workitem_active` コンテキストのインラインアクションとして表示（`$(git-branch)` アイコン）
...
- **反復アイテムフィルタリング** (`ado-ext.setIterationItemFilter`)
  - イテレーション内のワークアイテムに対してフィルタを適用

- **PR フィルタリング** (`ado-ext.setPrFilter`)
...
| iterationFilter    | 🔽 (filter)        | No          | sprintsIterationFilter\_{key} |
...
| pullRequestFilter  | 🔽 (filter)        | No          | pullRequestsFilter\_{key}     |
